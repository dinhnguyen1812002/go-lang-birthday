...
const letters = "Happy birthday"; // Define the message you want
...
/* Define more vibrant colors */
  ...
  const sourceHTML = document.getElementById("source").innerHTML;

  // Tách theo <br>
  const parts = sourceHTML.split(/<br\s*\/?>/i);
  ...
  function type() {
    if (line >= parts.length) return;

    if (char < parts[line].length) {
      target.innerHTML += parts[line].charAt(char);
      char++;
      ...
      line++;
      char = 0;
      setTimeout(type, 400); // delay giữa các dòng
    }
  }
